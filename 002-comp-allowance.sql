...
-- NOTE, added after this was run against production: the line below DOES
-- NOTHING. A column-level revoke cannot take a privilege from a role that holds
-- the table-level grant, and Supabase grants authenticated/anon UPDATE on every
-- table in public by default. It runs without error and changes nothing. The
-- real fix is 004-column-grants.sql. This line is kept only so the file still
