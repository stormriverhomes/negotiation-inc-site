...
const assertAccountAndBilling = () => {
  ...
  for (const f of ['desk.html','office.html']){
    ...
    /* 2800, not 1800: afterStripe grew two branches that both have to come
       BEFORE the wait — the unconfigured-account-layer answer and the
       session-expired-on-Stripe's-page answer — and the window is measured
       from the function's first character, so real prose in front of the loop
       pushed the loop out of view and the guard fired on a correct file. Widen
       it when the function honestly grows; never delete it. */
    const back = d.match(/async function afterStripe[\s\S]{0,2800}/);
...
(function(){
 ...
 /* ── UNLESS THE PAGE ALREADY KNOWS YOUR NAME ────────────────────────────
    This block is appended to EVERY page, and its fallback branch adds a
    "Elijah \\u2192" pill when it cannot find a "Sign in" link to rewrite. The
    office masthead has no such link — it carries its own chip, with the
    avatar, the name and Sign out — so the fallback fired and the phone
    header read "E  Elijah  (Elijah \\u2192)  Sign out": the name three times
    in four controls, on the one page where the account is the whole point.
    A page that already introduces you does not need introducing. */
 if(!got && !hdr.querySelector('#who,.who')){
...
async function authSignUp(email, password, name){
  ...
  try {
    /* ── THE PLAN TRAVELS IN THE CONFIRMATION LINK ────────────────────────
       Somebody who clicked "Start 14 days free" and had to make an account
       first goes to their inbox — and opens the mail on their PHONE about half
       the time. A different device has none of this browser's storage, so
       whatever we parked here is unreachable, and they arrive, confirm, sign
       in, and land on a free account with nothing anywhere remembering that
       they came here to pay.
       The link is the one thing that reaches the other device. So the chosen
       plan rides in it, and joinFromQuery picks it up on arrival exactly as if
       they had clicked the button there. Read inline rather than through
       joinHeld(), which lives in the billing block appended after this one.
       (The address must still be on Supabase's Redirect URLs allowlist —
       query strings are matched by prefix, so office.html?confirmed=1 covers
       it and no dashboard change is needed.) */
    const want = (() => {
      try { const o = JSON.parse(localStorage.getItem('ni-join-v1') || 'null');
            if (o && typeof o.plan === 'string' && o.plan) return o.plan; } catch(e){}
      try { return sessionStorage.getItem('ni-join') || ''; } catch(e){ return ''; }
    })();
    const back = 'office.html?confirmed=1' + (want ? '&join=' + encodeURIComponent(want) : '');
    const r = await fetch(\`\${SB.url}/auth/v1/signup\`, { method:'POST', headers: sbHead(),
      body: JSON.stringify({ email, password, data:{ name },
                             options:{ emailRedirectTo: authBack(back) },
...
async function afterStripe(){
  ...
  /* ── A PAYMENT IS NEVER MET WITH SILENCE ──────────────────────────────────
     This function used to OPEN with the PAY_ON() test, and PAY_ON() reads a
     value that arrives from /api/config a moment AFTER this runs. On the live
     site — where the Supabase settings are not baked into the page and must be
     fetched — it was therefore false every single time, and somebody who had
     just been charged came back to a page that said nothing whatsoever.

     The readiness wait at the bottom of this file fixes the timing. This
     ordering fixes the consequence: the query says a card was charged, so from
     here on every path ends in a sentence, even the path where the answer
     never arrives at all. */
  if (!PAY_ON()){
    payNote('Your payment went through. This page cannot reach the account service just now — '
          + 'reload in a minute, and email support@negotiationinc.com if it is still not here.', 'bad');
    return;
  }

  /* ── AND IF THE SESSION DIED WHILE THEY WERE ON STRIPE'S PAGE ─────────────
     A checkout can take a while — a card fetched from another room, a 3-D
     Secure step on a phone — and the token behind this page expires. They come
     back charged, with no session, and the poll below asks the server twenty
     times with nothing to ask WITH, then reports a confirmation that "has not
     reached us", which is both wrong and alarming. The account is fine and the
     subscription is fine; the only thing missing is a sign-in. Say that. */
  const signedNow = await (async () => {
    try { return !!(window.__authToken && await window.__authToken()); } catch(e){ return false; }
  })();
  if (!signedNow){
    payNote('Payment taken, and your subscription is being set up. '
          + 'Sign in again and everything it opens is open.');
    return;
  }
...
/* ── AND WHETHER THERE IS A SESSION, NOT WHETHER A CACHE IS WARM ───────────
   The record above is written by the boot pass, which is asynchronous. A
   person who signed in on another device, or cleared site data, or simply
   arrived faster than their profile did, has a perfectly good session and no
   record — and reading only the record calls them a stranger and parks their
   money. Asking for a token asks the one question that decides anything: the
   checkout call is going to need exactly this and nothing else. The record
   stays as the fallback, because a build with a stubbed auth layer has one. */
async function joinSignedIn(){
  try { if (window.__authToken){ const t = await window.__authToken(); if (t) return true; } } catch(e){}
  return !!joinAccount();
}

/* ── WHERE A CHOSEN PLAN WAITS WHILE SOMEBODY MAKES AN ACCOUNT ─────────────
   It waited in sessionStorage, which is per TAB. And the account-first flow —
   which is the flow, because the server will not start a checkout without a
   session — runs like this:

     click Start 14 days free  →  the office parks the plan, shows the door
     sign up                   →  "check your email"
     open the email            →  A NEW TAB, usually, often another device
     confirm, sign in          →  sessionStorage is empty. The plan is gone.

   So every customer who did not already have an account — the majority of
   them, on a product that launched yesterday — signed up because they wanted
   to pay, confirmed their address, and landed on a free account with nothing
   to tell them or us what they had come for. They have to find the plans page
   again and know to click the same button twice.

   localStorage survives the new tab. It also survives for a month, which is
   why the intent carries the moment it was made: inside a day it is the same
   journey and it continues by itself; older than that and it is OFFERED, with
   a button, because a page that redirects somebody to a payment screen for
   something they half-decided last week is worse than one that asks. */
const JOINSESS = 'ni-join';            // this tab, unchanged, still read first
const JOINLOCAL = 'ni-join-v1';        // this browser: { plan, at }
const JOIN_FRESH = 86400000;           // a day: still the same journey
const JOIN_KEEP  = 7 * 86400000;       // a week: after that it is not an intent
function joinPark(plan){
  try { sessionStorage.setItem(JOINSESS, plan); } catch(e){}
  try { localStorage.setItem(JOINLOCAL, JSON.stringify({ plan, at: Date.now() })); } catch(e){}
}
function joinForget(){
  try { sessionStorage.removeItem(JOINSESS); } catch(e){}
  try { localStorage.removeItem(JOINLOCAL); } catch(e){}
}
function joinHeld(){
  let s = null; try { s = sessionStorage.getItem(JOINSESS); } catch(e){}
  if (s && String(s).trim()) return { plan: String(s).trim().toLowerCase(), fresh: true };
  let o = null; try { o = JSON.parse(localStorage.getItem(JOINLOCAL) || 'null'); } catch(e){}
  if (!o || typeof o.plan !== 'string' || typeof o.at !== 'number') return null;
  const age = Date.now() - o.at;
  /* a clock that moved backwards is not a fresh intent */
  if (age < 0 || age > JOIN_KEEP){ joinForget(); return null; }
  return { plan: o.plan.trim().toLowerCase(), fresh: age < JOIN_FRESH };
}
/* the older intent, offered rather than acted on */
function joinOffer(plan){
  const say = plan === 'the office' ? 'The Office'
            : plan.charAt(0).toUpperCase() + plan.slice(1);
  payNote('You were starting ' + say + ' before you made your account.', null,
    { label: 'Pick up where I left off', fn: async () => {
        joinForget();
...
async function joinFromQuery(){
  ...
  const held = joinHeld();
  const want = plan || (held ? held.plan : '');
  /* an intent that arrived in this URL is as fresh as an intent gets */
  const fresh = !!plan || !!(held && held.fresh);
  if (!want) return;
  if (plan) try { history.replaceState(null, '', location.pathname); } catch(e){}
  const park = () => joinPark(want);
  /* ── AND THE ASKING IS NEVER MET WITH SILENCE EITHER ─────────────────────
     PAY_ON() was the first line of this function, and it is false until the
     answer from /api/config lands. So the sequence that every single "Start 14
     days free" button on the plans page produces — signed-in person arrives at
     office.html?join=underwriter — returned here, one line in, having done
     nothing and said nothing. They landed on their account and wondered where
     the checkout went. That is the whole bug, and it survived the previous fix
     because the previous fix was to a line this one never reached.

     Keep the shout for people who arrived ASKING to pay. A parked plan swept
     on an ordinary visit is housekeeping, not news. */
  if (!PAY_ON()){
    park();
    if (plan) payNote('Subscriptions cannot be started from this page just now. '
                    + 'Reload, and email support@negotiationinc.com if it happens twice.', 'bad');
    return;
  }
  const signed = await joinSignedIn();
  if (!signed){ park(); return; }
  /* older than a day: ask, do not act. See the note above joinPark. */
  if (!fresh){ joinOffer(want); return; }
  joinForget();
  /* Between the click and Stripe there is a page load, a config fetch, a token
     and a round trip — about a second, spent looking at a dashboard that is
     doing nothing visible. That second is where somebody clicks the button
     again, or decides it did not work. One sentence, and it is replaced by
     either Stripe or the reason there is no Stripe. */
  payNote('Opening checkout…');
...
async function joinAfterDoor(){
  try { if (typeof window.__authReady === 'function') await window.__authReady(); } catch(e){}
  if (!PAY_ON()) return;
  const held = joinHeld();
  if (!held) return;
  const signed = await joinSignedIn();
  if (!signed) return;               // the door fired before the session landed
  if (!held.fresh){ joinOffer(held.plan); return; }
  joinForget();
  payNote('Opening checkout…');
  const r = await startCheckout(held.plan);
  if (!r.ok) payNote(r.error, 'bad', r.portal ? PORTAL_ACTION : null);
}

/* ── WHAT THE WORDMARK MEANS ONCE SOMEBODY HAS AN ACCOUNT ──────────────────
   Signed out it is the sales page, which is right. Signed in it is a trapdoor:
   the largest, most-clicked control on the screen throws a customer back to a
   pitch they have already accepted, and they have to find their own way back
   in. Both rails were fixed for this months ago; the masthead sitting above
   them was not, on either page.

   Only the wordmark moves. The footer still says Home and still means home —
   a person who wants the sales page should be able to reach it. */
function wordmarkHome(){
  if (!joinAccount()) return;
  const marks = document.querySelectorAll('a.marklink, a.mark, a.rn-mark');
  for (let i = 0; i < marks.length; i++){
    const el = marks[i];
    /* no regex: this module is a template literal and a backslash does not
       survive the trip, which has already eaten one escape in this file */
    const h = (el.getAttribute('href') || '').split('#')[0].split('?')[0];
    if (h !== 'index.html' && h !== '/index.html' && h !== '/') continue;
    el.setAttribute('href', 'office.html');
    el.setAttribute('title', 'Your desk');
  }
}
window.__wordmarkHome = wordmarkHome;
...
/* ── NOTHING BELOW MAY BE ASKED BEFORE THE CONFIG ANSWER LANDS ─────────────
   These three lines used to run synchronously, right here, and every one of
   them opens by asking SB_ON() — which reads two values that arrive from
   /api/config a fetch later. Baked into the page (a preview build, a file://
   copy, every harness in this repo) the values are there at parse time and all
   three worked, which is exactly why every test passed. On the LIVE site they
   are deliberately not baked in, so all three ran a few milliseconds too early,
   found the account layer switched off, and returned without a word:

     authBoot        — no profile fetched, no plan, no sheet sync
     afterStripe     — a customer back from a real charge, told nothing
     joinFromQuery   — every "Start 14 days free" click, dropped in silence

   The door survived because it is invoked BY the readiness promise rather than
   before it, which is why signing in worked and paying never did.

   One await, in front of all of it. */
(async () => {
  try { if (typeof window.__authReady === 'function') await window.__authReady(); } catch(e){}
  try{ wordmarkHome(); }catch(e){}
  ...
  try{ await afterStripe(); }catch(e){}
  try{ await joinFromQuery(); }catch(e){}
})();
/* the wordmark needs no server — it reads one key — so it is also done at once,
   because a logo that changes meaning half a second late is a flicker */
try{ wordmarkHome(); }catch(e){}
/* ── AND THE SECOND BETWEEN THE CLICK AND STRIPE IS NOT DEAD AIR ───────────
   Arriving at office.html?join=underwriter, a customer watches their whole
   dashboard paint, sits looking at it for about a second while the config
   fetch and the token round trip finish, and is then thrown to Stripe. Nothing
   in that second says a thing, so it reads as a page that loaded the wrong
   screen and then twitched — which is exactly what "it takes me to the page
   but it's a little buggy" describes.
   The note needs no server: it is one localStorage read, so it lands with the
   first paint. Only for somebody who HAS an account, because for anybody else
   the honest next screen is the door, and promising a checkout in front of a
   sign-up form is the same lie in the other direction. */
try {
  if (location.search.indexOf('join=') >= 0 && joinAccount()) payNote('Opening checkout…');
} catch(e){}
})();
</script>
`;
