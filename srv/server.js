...
/* ── THE DESK API ────────────────────────────────────────────────────────
   Written and tested as its own tree under srv/desk/ and mounted here rather
   than run as a second service, because a second Render instance is a second
   $7 a month, a second set of environment variables to keep in step, and a
   second thing to forget to deploy. Everything it needs is already in this
   process's environment.

   It mounts BEFORE the static middleware and before the 404 — a route added
   after express.static is a route that never runs. Loaded lazily and inside
   a try so that a fault in the desk cannot take the marketing site down with
   it: the site is what sells; the desk is what a customer uses after. */
import { mountDesk } from './deskapi/server.mjs';
...
/* the desk's own routes — every /desk/* endpoint, mounted while the router
   still has somewhere to put them */
try {
  mountDesk(app, { log: (...a) => log('[desk]', ...a) });
} catch (e) {
  log('DESK FAILED TO MOUNT', String((e && e.message) || e).slice(0, 200));
}
